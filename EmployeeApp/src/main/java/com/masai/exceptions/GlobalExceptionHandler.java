...
public class GlobalExceptionHandler {
	...
	public ResponseEntity<MyErrorDetails> tutorialNotFoundException(EmployeeNotfoundException tnf ,WebRequest wr){
		MyErrorDetails err = new MyErrorDetails(LocalDateTime.now(), tnf.getMessage(), wr.getDescription(false));
		return new ResponseEntity<MyErrorDetails>(err, HttpStatus.BAD_REQUEST);
	}
	
	@ExceptionHandler(AddressNotfoundException.class)
	public ResponseEntity<MyErrorDetails> AddressNotfoundException(AddressNotfoundException tnf ,WebRequest wr){
		MyErrorDetails err = new MyErrorDetails(LocalDateTime.now(), tnf.getMessage(), wr.getDescription(false));
		return new ResponseEntity<MyErrorDetails>(err, HttpStatus.BAD_REQUEST);
	}
